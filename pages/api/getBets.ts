...
export default async function handler(
    ...
    if (req.method === 'GET') {
        if (authToken == config.laApiKey) {
            ...
            unique.map((el: any) => {
                let totalAvailableBets = 0
                meciuri.map((meci: any) => {
                    if (meci.day == el) {
                        totalAvailableBets++
                    }
                })
                availableDays.push({ day: el, totalAvailableBets: totalAvailableBets })
            })
            res.status(200).send({ meciuri: meciuri, availableDays: availableDays, timestamp: Date.now(), status: "success" })
        } else {
